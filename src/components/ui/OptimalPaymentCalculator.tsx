...
const formSchema = z.object({
  ...
  // Add slider value to the schema
...
export default function OptimalPaymentCalculator() {
  // Update state to hold the new, complex result object
  ...
  const balance = form.watch('balance'); // Watch balance for slider max value
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      const response = await fetch('/api/calculate-payment-date', {
        ...
        body: JSON.stringify({...values, extraPayment: extraPaymentValue[0]}),
      });
  ...
  // Configuration for the new Line Chart
  const projectionChartData = {
      labels: result?.projections.map(p => `${p.months} Month${p.months > 1 ? 's' : ''}`),
      datasets: [
          {
              label: 'Projected Savings ($)',
              data: result?.projections.map(p => p.projectedSavings),
              fill: true,
  ...
  const projectionChartOptions = {
    ...
    plugins: {
        ...
        title: { display: true, text: 'Long-Term Savings Projection' },
    },
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
                ...
                <FormItem>
                    ...
                    <FormControl>
                        <Slider
                            ...
                            onValueChange={setExtraPaymentValue}
                            className="w-full"
                        />
                    </FormControl>
                </FormItem>
      ...
      {result && !isLoading && (
        <CardFooter className="flex-col items-start gap-6 mt-6 border-t pt-6">
          <div className="w-full space-y-4">
              <h3 className="text-xl font-bold">Payment Scenarios</h3>
              <p>Based on your inputs, here&apos;s how different payment dates affect your interest charges this month.</p>
              ...
              <Table>
                  <TableHeader>
                      <TableRow>
                          ...
                          <TableHead className="text-right text-green-600">Savings</TableHead>
                      </TableRow>
                  </TableHeader>
                  <TableBody>
          ...
          <div className="w-full pt-6">
            {/* New Line Chart for Projections */}
      ...
      {error && !isLoading && (
        <CardFooter><p className="text-red-500">{error}</p></CardFooter>
      )}
    </Card>
  );
}
